	...
	if(isset($post) && !empty($post)) {
		...
		// SINGLE, NOT ATTACHMENT
			...
			// POST
				...
				$homeID = get_option("page_for_posts");
				$home_post = get_post($homeID);
				echo '<a href="'.get_permalink($homeID).'">'.$home_post->post_title.'</a>';
				$parents[] = array(
					'class' => '',
					'name'  => $post_type->labels->singular_name,
					'link'  => home_url("/").'/'.$slug['slug'],
		...
		$filtered_home = apply_filters('extra_arianne_home', $home);
		if (!empty($filtered_home)) {
			$home = $filtered_home;
		}

		$filtered_parents = apply_filters('extra_arianne_parents', $parents);
		if (!empty($filtered_parents)) {
			$parents = $filtered_parents;
		}

		$filtered_current_post = apply_filters('extra_arianne_current', $current_post);
		if (!empty($filtered_current_post)) {
			$current_post = $filtered_current_post;
		}

		$filtered_delimiter = apply_filters('extra_arianne_delimiter', $delimiter);
		if (!empty($filtered_delimiter)) {
			$delimiter = $filtered_delimiter;
		}
