...
function extra_wpcf7_ajax_loader () {
	return get_template_directory_uri() . '/assets/img/visu/loading.gif';
}
 ...
 * @param string $alt
 */
function get_extra_responsive_image($src = 0, $params= array(), $class = '', $alt = '') {
	...
	} else if(empty($src) || !filter_var($src, FILTER_VALIDATE_URL)) {
		throw new Exception(__('La source de l\'image n\'est pas valide', 'extra'));
	}
...
function extra_responsive_image($src = 0, $params= array(), $class = '', $alt = '') {
	echo get_extra_responsive_image($src, $params, $class, $alt);
}
?>